...
class Tiket_model extends CI_Model
{
    ...
    public function get_detail_event($id_event){
        ...
        public function get_detail_order($id){
            if(empty($id)){
                ...
                $session = $this->session->userdata('login');
                $this->db->select("*, b.event_name, b.price, sum(a.qty*b.price) as total");
                $this->db->from("tr_order a");
    ...
    function getorder($id){
		// $sql = "SELECT *, c.nama_jenis FROM tbl_kak a join ms_unit b on a.id_unit = b.id_unit join ms_jenis_vendor c on a.id_jenis_vendor = c.id_jenis WHERE a.id_kak = '{$id}';";
		$sql = "SELECT a.*, b.price, b.event_name, sum(a.qty*b.price) as total FROM tr_order a left join event b on a.id_event = b.id WHERE a.id = '{$id}';";
		$data = $this->db->query($sql);
		return $data->row_array();
    }
    
    function get($email,$handphone,$code){
		$sql = "SELECT id FROM tr_order WHERE email = '{$email}' and handphone = '{$handphone}' and code = '{$code}';";
		$data = $this->db->query($sql);
		return $data->row_array();
	}
}